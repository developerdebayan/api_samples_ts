...
export const todoList = async (req: Request, res: Response): Promise<Response> => {
...
export const deleteTodo = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        const user = await User.findOne({ _id: new ObjectId(id) });
        if (user) {
            const result = await Todo.deleteOne({ _id: new ObjectId(todoId), userId: user.id });
...
export const updateTodo = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        const user = await User.findOne({ _id: new ObjectId(id) });
        if (user) {
            const todo = await Todo.findOne({ _id: new ObjectId(todoId), userId: user.id });
